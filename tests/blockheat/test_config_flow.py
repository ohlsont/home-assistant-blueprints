...
@pytest.mark.asyncio
async def test_config_flow_creates_entry_with_merged_data(blockheat_env: SimpleNamespace) -> None:
    const = blockheat_env.const
    flow = blockheat_env.config_flow.BlockheatConfigFlow()
...
async def test_config_flow_uses_domain_filtered_entity_selectors(
    ...
    control_selector = _schema_value(result["data_schema"], const.CONF_CONTROL_NUMBER_ENTITY)
...
@pytest.mark.asyncio
async def test_config_flow_rejects_non_positive_boost_slope(
    blockheat_env: SimpleNamespace,
) -> None:
    const = blockheat_env.const
    flow = blockheat_env.config_flow.BlockheatConfigFlow()
    step_2 = await flow.async_step_user(_step1_user_input(const))
    assert step_2["type"] == "form"
    assert step_2["step_id"] == "tuning"

    boost_slope_validator = _schema_value(step_2["data_schema"], const.CONF_BOOST_SLOPE_C)
    assert boost_slope_validator(5) == 5.0

    with pytest.raises(vol.Invalid):
        boost_slope_validator(0)
