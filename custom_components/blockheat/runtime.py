...
class BlockheatRuntime:
    ...
    async def async_recompute(self, reason: str = "manual") -> dict[str, Any]:
        ...
        async with self._lock:
            ...
            fallback_active_effective, fallback_debug = await self._async_apply_fallback(
                now,
                policy_on_effective,
            )
    ...
    async def _async_apply_policy(self, now: datetime) -> dict[str, Any]:
        ...
        floor_temp = (
            self._state_float(floor_temp_sensor, default=0.0) if floor_temp_sensor else 0.0
        )
        ...
        if computation.should_turn_on:
            await self._async_call_entity_service("input_boolean", "turn_on", target_boolean)
            ...
            await self._async_call_entity_service("input_boolean", "turn_off", target_boolean)
    ...
    async def _async_apply_daikin(self, policy_on_effective: bool) -> dict[str, Any]:
        ...
        computation = compute_daikin(
            ...
            outdoor_temp_threshold=self._cfg_float(CONF_DAIKIN_OUTDOOR_TEMP_THRESHOLD, -10.0),
    ...
    async def _async_apply_floor(self, policy_on_effective: bool, now: datetime) -> dict[str, Any]:
        ...
        computation = compute_floor(
            ...
            min_switch_interval_min=self._cfg_int(CONF_FLOOR_MIN_SWITCH_INTERVAL_MIN, 15),
            now=now,
        )
